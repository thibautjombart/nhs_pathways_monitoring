...
## Temporary analyses for SPI-M / John

## generate for regions
counts_ccg_age <- pathways_recent %>%
  mutate(age = if_else(age == "70-120", "70+", age)) %>%
  incidence(
      date_index = date,
      groups = c(ccg_name, age),
      count = count,
      na_as_group = FALSE
  )

## add on day and weekday
counts_ccg_age <- counts_ccg_age %>%
  mutate(day_type = day_of_week(date_index),
         weekday = weekdays(date_index))
...
ccg_age_stats %>%
  mutate(group = ccg, ccg = sub("-[0-9]+.*$", "", ccg),
         age = sub("[^[:digit:]]+-", "", group))  %>%
  group_by(age, n_recent_increase) %>%
  summarise(count = n()) %>%
  group_by(age) %>%
  mutate(p = 100*count / sum(count)) %>%
  ggplot(aes(x = n_recent_increase, y = p)) +
  geom_col() +
  facet_wrap(~ age) +
  theme_bw() +
  labs(x = "Days of increase (last week)",
       y = "% of CCGs",
       title = "Increase in NHS 111/999 potential cases by CCG and age group")
