...
```{r descriptive-hidden, eval = FALSE}

## this one is used to check the data content

x <- pathways %>%
  group_by(date, site_type, age) %>%
...
x %>%
  ...
  theme_bw() +
  labs(x = "",
       y = "Daily potential COVID-19 cases",
       title = "NHS pathways reports since 1st August") +
...
res_england <- asmodee(counts_england,
                       models,
                       method = evaluate_aic,
                       alpha = 0.05,
                       fixed_k = 7)
