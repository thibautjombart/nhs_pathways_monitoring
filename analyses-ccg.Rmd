...
CCG <- ccg_shape_aligned %>%
  ...
  select(ccg20nm, ccg, n_recent_outliers, n_recent_increases, n_recent_decreases, r, r_lower, r_upper, geometry) %>%
  mutate(
...
the ASMODEE predictions for. Selecting the CCG of interest will present the ASMODEE plot
for that CCG
...
Selecting the growth rate radial will display the growth rate for each CCG. Selecting the CCG
of interest will present the point estimate of *r* and the upper and lower confidence intervals.
...
factpal <- colorFactor(palette.colors(n = 7, palette = "Set 1", alpha = c(0.2, 1, 1, 1, 1, 1, 1)), CCG$`Days increased`, alpha = T) 
contpal <- colorNumeric(palette = "Spectral",  domain = CCG$r, reverse = T)

r_plot <- vector("list", length(CCG$`CCG Name`))
names(r_plot) <- CCG$`CCG Name`

for(i in 1:length(CCG$`CCG Name`)) {
  r_plot[[i]] <- ggplot(tibble(CCG[i,]), aes(y = `CCG Name`, x = r)) +
    geom_errorbar(aes(xmin = r_lower, xmax = r_upper)) +
    geom_point() +
    scale_y_discrete(labels = substr(CCG$`CCG Name`[i], 5, nchar(CCG$`CCG Name`[i])-4)) +
    theme_bw() +
