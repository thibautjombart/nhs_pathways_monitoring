...
counts_ccg_all <- counts_ccg_all %>%
  mutate(day_type = day_of_week(date),
         weekday = weekdays(date)) # changed to date from date_index #
...
### Mapping CCG outliers

This map highlights the CCGs with changes detected by ASMODEE over the last 7 days

```{r ccg_map, layout="l-screen-inset shaded"}
...
CCG <- ccg_shape_aligned %>%
  ...
  select(ccg20nm, n_outliers_recent, n_outliers, n_recent_increase, n_recent_decrease, geometry) %>%
  mutate(n_outliers_recent = factor(n_outliers_recent, levels = c("0", "1", "2", "3", "4", '5', "6", "7")),
         n_recent_increase = factor(n_recent_increase, levels = c("0", "1", "2", "3", "4", '5', "6", "7")),
         n_recent_decrease = factor(n_recent_decrease, levels = c("0", "1", "2", "3", "4", '5', "6", "7"))) %>%
  rename("CCG Name" = "ccg20nm",
         "Number of days outlying (last week)" = "n_outliers_recent",
         "Number of outliers (total)" = "n_outliers",
         "Number of days increased" = "n_recent_increase",
         "Number of days decreased" = "n_recent_decrease")

increase <- mapview(CCG, 
        zcol = "Number of days increased", 
        legend = T)

decrease <- mapview(CCG, 
        zcol = "Number of days decreased", 
        legend = T)

together <- sync(increase, decrease)

mapview(CCG, 
        zcol = "Number of days outlying (last week)", 
        legend = T)
```

Plot isn't showing for some reason
