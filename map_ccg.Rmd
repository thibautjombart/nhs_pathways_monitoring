...
counts_ccg_all <- counts_ccg_all %>%
  mutate(day_type = day_of_week(date_index),
         weekday = weekdays(date_index)) # changed to date from date_index #
...
These maps highlight the CCGs with changes detected by ASMODEE over the last 7 days
...
CCG <- ccg_shape_aligned %>%
  ...
  mutate(n_recent_increase = factor(n_recent_increase),
         ...
         ccg = stringr::str_replace(ccg, "nhs_", "")) %>% # this should be able to join to the names in res_ccg_all
...
#### CCGs with outliers of increased calls
``` {r increased_ccgs, results="markup", layout="l-screen-inset shaded"}
tmap_mode("view")

increased
```

#### CCGs with outliers of decreased calls
``` {r decreased_ccgs, results="markup", layout="l-screen-inset shaded"}
decreased
```

#### CCGs with number of outlier days in the last week

Selecting the CCG will pull up the ASMODEE prediction plot for that CCG

``` {r combined, results="markup", layout="l-screen-inset shaded"}
CCG <- CCG %>%
  select(-ccg)

mapview(CCG, 
       zcol = "Number of outlier days (last week)", 
       legend = T,
       popup = popupGraph(plot(res_ccg_all[[1]])))
